...
func _store_decision(id: int, candidates: Array, chosen_pos: Vector2i, metric: String) -> void:
	...
	var weights := _base_weights(origin, chosen_pos)
...
func _apply_candidate_weights(weights: Dictionary, candidates: Array, metric: String) -> void:
	...
	for row in candidates:
		...
		if metric == "visits":
			...
			var score := float(row.get("score", 0.0))
			var spread := max_score - min_score
			if spread > 0.001:
				norm = (score - min_score) / spread
			else:
				norm = (score + max_abs) / (2.0 * max_abs)
...
func _draw() -> void:
	...
	for agent in _agents:
		...
		var color := _agent_color(agent)

		for dir in DIRS:
			var pos := origin + dir
			var weight := clampf(float(weights.get(pos, MIN_WEIGHT)), MIN_WEIGHT, 1.0)
...
func _draw_tile_choice(pos: Vector2i, color: Color, weight: float, chosen: bool) -> void:
	...
	var in_bounds := tile != null
	if not in_bounds:
		return
	var walkable := in_bounds and tile.walkable
	var alpha := 0.12 + weight * 0.26
	if chosen:
		alpha = 0.58
	elif not walkable:
		alpha *= 0.38

	var x := float(pos.x * TILE_SIZE)
	var y := float(pos.y * TILE_SIZE)
	var rect := Rect2(x + PAD, y + PAD, TILE_SIZE - PAD * 2.0, TILE_SIZE - PAD * 2.0)
	...
	if chosen:
		...
		draw_arc(center, TILE_SIZE * (0.32 + pulse * 0.05), 0.0, TAU, 24,
			Color(color.r, color.g, color.b, 0.70), 2.0)
	elif not walkable:
		draw_line(rect.position + Vector2(5.0, 5.0), rect.end - Vector2(5.0, 5.0),
			Color(color.r, color.g, color.b, 0.30), 1.0)

func _agent_color(agent) -> Color:
	match agent._role:
		"rusher_red":
