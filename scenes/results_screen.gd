...
func _draw_professional_results() -> void:
	...
	var agent_results: Array = _ranked_results(_result.get("agent_results", []))
	var card_gap := 24.0
	...
	_draw_results_summary(Rect2(header.position.x, footer_y, content_w, 122.0), outcome, font, alpha)
...
func _draw_results_agent_card(ar: Dictionary, rect: Rect2, font: Font, alpha: float) -> void:
	...
	var eliminated: bool = ar.get("eliminated", false)
	var camera_hits: int = int(ar.get("camera_hits", 0))
	var wall_hits: int = int(ar.get("wall_hits", 0))
	var best_progress_cells: int = int(ar.get("best_progress_cells", 0))
	var dog_zone_time: float = float(ar.get("dog_zone_time", 0.0))
	...
	var moves: int = int(ar.get("moves", 0))
	var waits: int = int(ar.get("waits", 0))
	var abilities: int = int(ar.get("abilities", 0))
	var sprints: int = int(ar.get("sprints", 0))
	var captures_taken: int = int(ar.get("captures", 0))
	var elimination_tick: int = int(ar.get("elimination_tick", -1))
	var escape_rank: int = int(ar.get("escape_rank", -1))
	var is_active: bool = bool(ar.get("is_active", false))
	var capture_cd: int = int(ar.get("capture_cooldown_ticks", 0))
	...
	draw_string(font, rect.position + Vector2(pad, 188.0), "RAW %d   ·   PERF %d   ·   STL %d" % [int(raw_score), int(performance), int(stealth)],
		HORIZONTAL_ALIGNMENT_LEFT, rect.size.x - pad * 2.0, 12, Color(C_TEXT.r, C_TEXT.g, C_TEXT.b, 0.92 * alpha))
	if role == "police":
		draw_string(font, rect.position + Vector2(pad, 206.0), "CAP %d  DOG %d  CCTV %d  FIRE %d  ESC %d" % [captures_made, dog_assists, cctv_assists, fire_assists, escapes_allowed],
			HORIZONTAL_ALIGNMENT_LEFT, rect.size.x - pad * 2.0, 10, Color(C_DIM.r, C_DIM.g, C_DIM.b, 0.88 * alpha))
	else:
		draw_string(font, rect.position + Vector2(pad, 206.0), "STEALTH %d  PROG %d  DOG %.1fs  CCTV %d  WALL %d" % [int(stealth), best_progress_cells, dog_zone_time, camera_hits, wall_hits],
			HORIZONTAL_ALIGNMENT_LEFT, rect.size.x - pad * 2.0, 10, Color(C_DIM.r, C_DIM.g, C_DIM.b, 0.88 * alpha))
	draw_string(font, rect.position + Vector2(pad, 224.0), "M %d  W %d  AB %d  SP %d  CAPTURED %d" % [moves, waits, abilities, sprints, captures_taken],
		HORIZONTAL_ALIGNMENT_LEFT, rect.size.x - pad * 2.0, 10, Color(C_DIM.r, C_DIM.g, C_DIM.b, 0.88 * alpha))
	draw_string(font, rect.position + Vector2(pad, 242.0), "ESC_RANK %s  ELIM_TICK %s  ACTIVE %s" % [_int_or_dash(escape_rank), _int_or_dash(elimination_tick), "YES" if is_active else "NO"],
		HORIZONTAL_ALIGNMENT_LEFT, rect.size.x - pad * 2.0, 10, Color(C_DIM.r, C_DIM.g, C_DIM.b, 0.88 * alpha))
	draw_string(font, rect.position + Vector2(pad, 260.0), "CAP_CD %d  ALERT %d%%" % [capture_cd, int(alert_level * 100.0)],
		HORIZONTAL_ALIGNMENT_LEFT, rect.size.x - pad * 2.0, 10, Color(C_DIM.r, C_DIM.g, C_DIM.b, 0.88 * alpha))
	draw_string(font, rect.position + Vector2(pad, 278.0), "HP %.1f  ST %.1f  CCTV_HITS %d  WALL_HITS %d" % [hp, stamina, camera_hits, wall_hits],
		HORIZONTAL_ALIGNMENT_LEFT, rect.size.x - pad * 2.0, 10, Color(C_DIM.r, C_DIM.g, C_DIM.b, 0.88 * alpha))
...
func _draw_results_summary(rect: Rect2, outcome: String, font: Font, alpha: float) -> void:
	...
	if ranked.size() >= 3:
		...
		var line := "1) %s %d   ·   2) %s %d   ·   3) %s %d" % [
			_role_short(str(r0.get("role", "?"))), int(float(r0.get("raw_score", 0.0))),
			_role_short(str(r1.get("role", "?"))), int(float(r1.get("raw_score", 0.0))),
			_role_short(str(r2.get("role", "?"))), int(float(r2.get("raw_score", 0.0))),
		]
		_draw_results_centered_text(font, line, Vector2(rect.get_center().x, rect.position.y + 66.0), 13, Color(C_TEXT.r, C_TEXT.g, C_TEXT.b, 0.84 * alpha))
	_draw_results_centered_text(font, "Replay to test a new generated facility.",
		Vector2(rect.get_center().x, rect.position.y + 90.0), 13, Color(C_DIM.r, C_DIM.g, C_DIM.b, 0.72 * alpha))
...
func _ranked_results(agent_results: Array) -> Array:
	...
	out.sort_custom(func(a, b):
		var sa: float = float(a.get("raw_score", 0.0))
		var sb: float = float(b.get("raw_score", 0.0))
		if is_equal_approx(sa, sb):
			return float(a.get("performance", 0.0)) > float(b.get("performance", 0.0))
		return sa > sb
	)
	return out
