...
func _process(delta: float) -> void:
	...
	if _show_options:
		for i in range(_option_rects.size()):
			if _option_rects[i].has_point(_mouse_pos):
				_hover_opt = i
				break
	queue_redraw()
...
func _input(event: InputEvent) -> void:
	if event is InputEventMouseButton \
		...
		if _show_options and _hover_opt >= 0:
			_toggle_option(_hover_opt)
			return
		match _hover_btn:
			BTN_PLAY: _start_game()
			BTN_OPTIONS: _show_options = not _show_options
...
func _unhandled_key_input(event: InputEvent) -> void:
	...
	match event.keycode:
		...
		KEY_O: _show_options = not _show_options
...
func _draw_professional_title() -> void:
	...
	var panel_w := minf(760.0, vp.size.x - 88.0)
	var panel_h := 640.0
	...
	if font != null:
		...
		_draw_centered_text(font, "DUAL ESCAPE SHOWDOWN", Vector2(cx, panel.position.y + 154.0), 22, C_WARNING)
	...
	var opt_y := play_y + 72.0
	var quit_y := opt_y + 72.0
	_btn_rects[BTN_PLAY] = Rect2(btn_x, play_y, btn_w, btn_h)
	_btn_rects[BTN_OPTIONS] = Rect2(btn_x, opt_y, btn_w, btn_h)
	...
	_draw_menu_button(_btn_rects[BTN_OPTIONS], "OPTIONS", BTN_OPTIONS == _hover_btn, C_WARNING, font)
	_draw_menu_button(_btn_rects[BTN_QUIT], "QUIT", BTN_QUIT == _hover_btn, Color(0.78, 0.28, 0.24), font)

	if _show_options:
		_draw_options_panel(panel, font)

	if font != null:
		_draw_centered_text(font, "Enter / Space to play    O to open options    Esc to quit",
...
func _draw_title_panel(panel: Rect2, pulse: float) -> void:
	...
	draw_line(panel.position + Vector2(28.0, 560.0), panel.position + Vector2(panel.size.x - 28.0, 560.0),
...
func _draw_options_panel(panel: Rect2, font: Font) -> void:
	if font == null:
		return
	_option_rects.clear()
	var rect := Rect2(panel.position.x + 120.0, panel.position.y + 540.0, panel.size.x - 240.0, 82.0)
	draw_rect(rect, Color(0.02, 0.04, 0.06, 0.92))
	draw_rect(rect, Color(0.18, 0.42, 0.60, 0.46), false)
	_draw_centered_text(font, "OPTIONS", Vector2(rect.get_center().x, rect.position.y + 16.0), 11, C_WARNING)
	...
	var w: float = (rect.size.x - 24.0 - 10.0 * 5.0) / 6.0
	var y: float = rect.position.y + 24.0
	for i in range(items.size()):
		var r := Rect2(rect.position.x + 12.0 + float(i) * (w + 10.0), y, w, 44.0)
		_option_rects.append(r)
		var hover: bool = i == _hover_opt
		var on: bool = values[i]
		var accent: Color = C_HIGHLIGHT if on else Color(0.46, 0.54, 0.64)
		draw_rect(r, Color(accent.r * 0.18, accent.g * 0.18, accent.b * 0.18, 0.92 if hover else 0.72))
		draw_rect(r, Color(accent.r, accent.g, accent.b, 0.85 if hover else 0.42), false)
		_draw_centered_text(font, items[i], Vector2(r.get_center().x, r.position.y + 15.0), 9, C_DIM)
		_draw_centered_text(font, "ON" if on else "OFF", Vector2(r.get_center().x, r.position.y + 33.0), 11, accent)
